defmodule Ledger.Transactions do
  ...
  def count_transactions(filter) do
    query =
      from t in base_query(filter.tag),
        select: count(),
        where: ^filter_account(filter),
        where: ^filter_search(filter.search)
  ...
  """
  @spec count_future_transactions(filter :: %TransactionFilter{}, cutoff :: Date.t()) ::
          Ecto.Query.t()
  ...
  def filter_account_scope(%{account_scope: "active"}) do
    ...
    dynamic(
      [t],
      t.account_id in subquery(accounts) or
        (is_nil(t.account_id) and
           t.destination_id in subquery(accounts)) or is_nil(t.destination_id)
    )
  end
      ...
      true ->
        dynamic(
          ...
          t.id in subquery(
            from(row in Fts,
              select: row.id,
              where: fragment("transactions_fts MATCH ?", ^fts_escape(search))
            )
          )
        )
    end
  ...
  def balance_map(deposits, withdrawls) do
    deposit_map =
      Enum.into(deposits, %{}, fn %{account_id: account_id, total_amount: total_amount} ->
        {account_id, total_amount}
      end)

    withdrawl_map =
      Enum.into(withdrawls, %{}, fn %{account_id: account_id, total_amount: total_amount} ->
        {account_id, total_amount}
      end)
  ...
  def get_transaction!(id) do
    ...
    Repo.one!(
      from t in Transaction,
        ...
        where: t.id == ^id,
        preload: [:tags, account: ^account_preload, destination: ^account_preload]
    )
  end
